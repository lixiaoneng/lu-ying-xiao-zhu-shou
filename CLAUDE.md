...
- 装备大本营使用 **Supabase Auth 邮箱账号**登录
- `equipment_items.user_id` 直接指向 `auth.users.id`
- 当前阶段**不创建** `profiles` / `user_identities` 表——暂无昵称、头像、多身份绑定、微信登录、小程序登录需求

### 未来多登录方式的前置要求

如果未来接入微信登录、公众号授权、小程序登录、手机号登录等，**必须先完成以下设计，才能动代码**：

1. 设计并创建 `profiles` 表（统一内部用户档案）
2. 设计并创建 `user_identities` 表（绑定各外部登录方式与内部 user_id 的映射）
3. 确保所有外部登录最终映射到同一个内部 `user_id`，用户邮箱账号与微信账号共享同一份装备数据

### 绝对禁止（未经明确批准）

| 禁止操作 | 原因 |
|----------|------|
| 把微信 openid / unionid 直接写入 `equipment_items.user_id` | 破坏数据归属，导致无法与邮箱账号合并 |
| 绕过 Supabase Auth 自行创建另一套用户 ID 体系 | 产生数据孤岛，身份无法统一 |
| 在身份绑定层缺失的情况下上线任何多登录方式 | 用户邮箱账号和微信账号将各自拥有独立装备库，日后无法合并 |

### 与当前上线范围的关系

微信登录 / 小程序登录的**前置任务**是完成 `profiles + user_identities` 的设计与评审，**不属于当前上线范围**。在该设计通过确认前，不得开始实现任何多登录方式。
...
以下情况必须先运行 `/handoff`，再关闭窗口或重开对话：

- 上下文窗口即将用尽（出现红色警告）
- 一个功能阶段开发完成
- 发现 bug 但还没修，需要暂停

`/handoff` 会自动更新 `PROJECT_STATUS.md`、运行 build 验证、输出交接摘要。
...
`/start-task` 会自动读取 CLAUDE.md + PROJECT_STATUS.md + git 状态，输出任务计划，等待确认后再开发。

**不要在未运行 `/start-task` 的情况下直接开始改代码。**

### 上下文管理原则

- 上下文红色时不做大规模开发，只做收尾和交接
- 每次 commit 粒度要小，便于上下文切换时快速恢复
- `PROJECT_STATUS.md` 是状态的单一来源，优先于对话记忆

### 文件位置

- 交接状态：`PROJECT_STATUS.md`（根目录）
- 交接命令：`.claude/commands/handoff.md`
- 启动命令：`.claude/commands/start-task.md`
